...
| Solução .NET, qualidade e TDD inicial | 5% | 100% | 5% | Solução compilável e 352 testes automatizados verdes, incluindo abertura de produto, wizard, configuração real, hierarquia editorial da tela Início, do detalhe e do histórico de reuniões, ciclo de vida seguro dos filtros, estabilidade geométrica da navegação, cache local de agenda, regressões de contraste, busca local completa, exportação PDF/DOCX, publicação Obsidian, texto selecionável, ata narrativa, lembretes locais, cartões acionáveis, qualidade de transcrição, OBS, Docker, Tray, Worker black box, Desktop real, observabilidade, detecção local e release Windows |
...
| Experiência de configuração e abertura | SPEK 100% concluída | Splash vetorial, wizard de três passos, formulário real persistido, motion respeitando o Windows e ícones de gravação cobertos pela suíte Release ([[SPEK-058 Experiencia de Configuracao e Abertura]]) |
