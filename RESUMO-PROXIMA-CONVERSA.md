...
Desktop: as SPEKs 027 a 031 entregaram uma janela WinForms nativa com o Design System `Command Deck`. O modo normal consulta reuniões, jobs e os 500 eventos operacionais mais recentes nos SQLite locais, inicia e encerra a gravação pelos casos de uso, acompanha o Worker por polling de dois segundos e abre artefatos por caminhos persistidos. O journal isolado preserva 12 códigos correlacionados por 14 dias e nunca armazena mensagem livre de exceção, áudio, transcrição, ata, prompt, segredo ou caminho pessoal. Concorrência, cancelamento, reinício e bancos legados não deixam uma gravação órfã bloqueando o produto. O modo simulado permanece isolado em `--poc-desktop`.

Próximo incremento: revisar e aprovar a SPEK-032 para captura instantânea orientada por sinais locais, sem iniciar gravação automaticamente sem confirmação do usuário.
